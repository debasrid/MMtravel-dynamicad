...
var common = (function() {
   ...
   * @param {element} price Regular price (displays with sale price).
   */
  function displayCorrectPrices(defaultPrice, salePrice, price) {
    alert();
    var priceValues = {
    ...
    if (isSalePriceEnabled_(priceValues)) {
      showSalePrice(true, defaultPrice, salePrice, price);
    } else {
      showSalePrice(false, defaultPrice, salePrice, price);
    }
  }
   ...
   * @param {?element} price Regular price (displays with sale price).
   */
  function showSalePrice(state, defaultPrice, salePrice,price) {
    utils.showElement(defaultPrice, !state);
    utils.showElement(salePrice, state);
    utils.showElement(price, state);
  }
  ...
  function isSalePriceEnabled_(item) {
    var isEnabled = item.salePrice && item.price &&
      !utils.isEmptyString(item.salePrice) &&
      !utils.isEmptyString(item.price) &&
      utils.stringValuesAreUnique(item.salePrice, item.price);
    return isEnabled;
  }
